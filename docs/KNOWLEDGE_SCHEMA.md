...
- taxonomy: `scientificName`/`japaneseNames`/`englishNames`は既存`Plant`から転記。`family`/`genus`は**カタログ全種、確立した植物学的知識に基づき新規に編集レベルで付与**（`reviewStatus: 'editorial'`。PR11時点は50種、PR24で75種に拡充、以後も新規種追加のたびに追随）。`taxonIds`（GBIF/POWO/iNaturalist/Pl@ntNet/YList）は実API連携なしに推測できないため**全件空**（PR14で実装）
...
テスト: `src/data/__tests__/plantDefinitions.test.ts`（10件）— カタログ全種のfamily/genus充足、taxonIds/sourceRefsが空であること、danger→safety.levelの正しさ、look-alikeグラフの双方向性、toxicPartsの過剰記載が無いことを検証（新規種追加時も自動的に対象になる）。
...
- **morphology詳細**（葉の形/花弁数等）：カタログ全種分の構造化データを確度高く用意するには出典付きの追加調査が必要。現状は`notes`のみのスパースな型に留めている。
