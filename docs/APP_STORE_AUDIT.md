...
- ~~撮影写真の永続化~~: **PR17で対応済み**。観察を保存するタイミングで `expo-file-system` の `documentDirectory` へコピーするようになり、OSのキャッシュ削除による画像消失リスクが解消（`src/utils/observationPhotoStorage.ts`）。Web はキャッシュ/永続の区別が無いため対象外（no-op）。
- ~~ESLint / テストの導入~~: **PR15で対応済み**（`eslint-config-expo`、CIに追加）。
- **未使用コードの整理**: 一部の未使用スタイル・`Colors` キーが残存（無害）。

### ⚠️ 新規発見（PR17検証中）: `Alert.alert` は Web ターゲットで無音に失敗する
`react-native-web` は `Alert.alert` を実装しておらず、Web（`npm run build:web`／Vercelデプロイ）では確認ダイアログ・選択肢ボタンが一切表示されません（ボタンに紐づくコールバックも呼ばれない）。影響範囲: メモ削除・すべてのデータを削除（`profile.tsx`）、養生メモ削除（`plant/[id].tsx`）、未特定のまま記録する選択肢・再訪日時選択（`scan.tsx`/`plant/[id].tsx`、PR17で追加）、共有前の確認（`ShareCard.tsx`）。
**iOS/Android実機では`Alert.alert`はOSネイティブのダイアログとして正常動作するため、App Store提出（主要ターゲット）には影響しません。** Web版の体験を揃える場合は、これらの呼び出しをクロスプラットフォームな独自モーダルコンポーネントへ置き換える改修が必要（8箇所、複数ファイルにまたがるため独立したPRとして扱うのが適切）。
