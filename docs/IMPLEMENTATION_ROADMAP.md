...
## PR36 スコープ（本PR、PR35自己レビュー(4巡目)で発見した2件の修正＋PR35文書の訂正）
PR35もマージ前に第三者レビューを受けていなかったため、同じ8観点の並列コードレビューをPR35自身の差分に対して実施した（PR24-32→PR33→PR34→PR35に続く4巡目、ユーザーの「さらに見ておいてください」という依頼に基づく）。今回はゲート計算ロジック（`determineMaxGate`/`isUseUnlocked`/`isCategoryUnlocked`）自体は「収束した」との評価で一致し、見つかったのはPR35自身が導入した表示バグ1件と、PR35の対応で見落としていた重複1件のみだった。

含む（severity順）:
- **【PR35が導入した回帰】`app/(tabs)/index.tsx`のspotlightカードで、GREENラベルの折り返し対策が不十分だった**: PR35で追加した`numberOfLines={1}`は、`spotlightDangerText`にもその親`spotlightDangerBadge`にも幅の制約が無かったため実質何もしておらず、実際にはみ出しを防いでいたのは`spotlightCard`側の`overflow: 'hidden'`のみだった。しかも`spotlightCard`は`alignItems: 'center'`（中央寄せ）のため、バッジ幅がカード幅（116px）を超えると左右**両端**が均等に切り取られる形になり、「一般に食用とされる」（GREEN、9文字）のような長いラベルは先頭・末尾の文字が両方欠けた読めない表示になり得た（`numberOfLines`が正しく機能していれば末尾のみが「…」で省略されるはずだった）。`spotlightDangerText`に`maxWidth: 68`（カード幅からパディング・ドット・gapを差し引いた実際の表示可能幅）を明示的に指定し、`numberOfLines`/`ellipsizeMode="tail"`が実際に効くようにして修正。合わせて、この修正によりバッジ自体がカード幅を超えなくなったため、`spotlightCard`の`overflow: 'hidden'`（このカードにこれまで無かった新規追加で、iOSのドロップシャドウを切り取ってしまう副作用も指摸されていた）は不要になり削除した
- **`src/components/DangerBadge.tsx`／`dotColor`のハードコピー4箇所**: `app/(tabs)/index.tsx`（2箇所）・`app/plant/[id].tsx`・`app/(tabs)/zukan.tsx`の危険度ドット色が、いずれも`DANGER_CONFIG[danger].dotColor`と完全一致する値（RED `#E53935`/YELLOW `#F9A825`/GREEN `#43A047`）を独自にハードコピーしていた。値が完全一致することを確認済みのため、視覚的な差分が一切発生しない安全な統合として、`DangerBadge.tsx`から新設の`DANGER_DOT_COLOR`をexportし、4箇所すべてをそれに置き換えた

含まない（今回判断・意図的にスコープ外、理由を明記）:
- **`app/(tabs)/profile.tsx`・`app/(tabs)/zukan.tsx`統計欄の「一般食用」「要注意」「危険」等のラベル**: レビューで「PR35が直したのと同じ重複バグでは」との指摘があったが、精査の結果これはバグではないと判断した。これらは特定の1個体の危険度を主張する表示（PR35で修正したバグの実体）ではなく、「GREEN分類の発見種は何種か」を示す統計カテゴリの見出しであり、狭いスタッツグリッド向けに意図的に短縮された表記（例:「危険」2文字 vs 正規ラベル「危険・有毒」5文字）。`DANGER_LABEL`の正規（より長い）文言に統一すると、今回spotlightカードで実際に発生した幅はみ出しバグをこの2画面に新たに持ち込むリスクの方が高いため、変更しないことを選択した
- **`app/plant/[id].tsx`・`app/(tabs)/index.tsx`の`bg`/`border`色のハードコピー**: PR35では「`DANGER_CONFIG`とは異なる意図的なアクセント色である可能性がある」と推測してスコープ外としていたが、4巡目のレビューで実際の値を突き合わせた結果、この推測は誤りだった可能性が高いと判明した——例えば`recentCard`のborder色はRED値のみ`DANGER_CONFIG.RED.border`と完全一致し、YELLOW/GREENは不一致という「3つのうち1つだけ偶然一致」という分布は、意図的な独自パレットというより、初期実装時の不完全なコピーによるドリフトである可能性の方が高い。ただし`dotColor`と異なりYELLOW/GREENの値は実際に異なるため、統一すれば実際に見た目が変わる（視覚確認なしに機械的に置換できない）。このセッションの実行環境では引き続きExpo web devサーバーが起動できず視覚確認ができないため、本PRでは「意図的な設計」という誤った前提を訂正する記録のみ残し、実際の統合は次回、実機/ブラウザでの目視確認ができるセッションに持ち越す
- **重複再発防止のためのESLintルール**（`.danger`への三項演算子/switch分岐を機械的に禁止する`no-restricted-syntax`）: 4巡目のレビューで提案されたが、実際に`grep`で検証したところ`.danger === '...'`を使うコードは30箇所以上あり、大半（真偽値の算出、`.filter()`条件、`if`文によるフロー制御等）はこのバグと無関係な正当な用途だった。提案されたセレクタ（`.danger`に対する任意の三項演算子/switchを禁止）はこれらすべてを誤検知するため、そのままでは導入できない。「両分岐が色・ラベルの文字列/hexリテラルである場合のみ禁止する」といった、より精密なセレクタの設計が必要であり、本PRの範囲を超えるため見送り、次回への申し送り事項として記録するに留める

## 検証（PR36）
- `npm run check`（typecheck + lint + jest 115件）green。ゲート計算ロジックは変更していないため既存テストの追加・変更は無し
- Expo web devサーバーは本セッションでも起動できなかったため、spotlightカードの幅はみ出し修正は、カード幅・パディング・ドット・gapの実測値からテキストの実際の表示可能幅（約68px）を計算した上での`maxWidth`指定によって対応した——`overflow: 'hidden'`による曖昧な防御ではなく、`numberOfLines`/`ellipsizeMode`が設計通り機能する明示的な幅制約であるため、目視確認なしでも動作原理として正しいと判断できる。ただし実機/ブラウザでの最終確認は引き続き申し送り事項とする
- `dotColor`統合は値が完全一致することをコード上で確認済みのため、視覚差分が発生しない安全な機械的置換であり、追加の視覚確認は不要と判断した
